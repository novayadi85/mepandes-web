jQuery(document).ready(function() {
  ...
  jQuery(document).on('click', '[data-menu]', function (e) {
      ...
      if (window.innerWidth > 720) {
          ...
          // Remove active class from all menu-holder elements within the same ul
          $clickedLi.closest('ul').find('.menu-holder').removeClass('active');

          if (jQuery(this).hasClass('has-children')) {
              var $menuHolder = $clickedLi.find('> .menu-holder');
              if ($menuHolder.length > 0) {
                  console.log('has children..', jQuery(this))
                  jQuery(this).find(' > .menu-holder').addClass('active');
                  e.preventDefault();
                  return;
              }
      ...
      if (menuId) {
          ...
          showContent(menuId, false);
          setTitle();
          if (!jQuery(".drawer--menu").hasClass('open')) {
              openDrawer();
          }
  ...
  function removeActiveFromClass() {
      jQuery('.drawer--contents').find('.menu-holder.active:last').removeClass('active');
  }
  ...
  function goBack() {
      ...
      if (jQuery('.drawer--contents').find('.menu-holder.active').length > 0) {
